...
mprof run --include-children granian --interface asgi main:app --host "$API_HOST" --port "$API_PORT" > memory_plots/log_api.txt &
echo "API running"  
sleep 10;
...
echo "test finished"
sleep 30;
mprof plot -o memory_plots/memory_granian
cp *.dat memory_plots
echo "plot created"
